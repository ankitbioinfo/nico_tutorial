...
def gene_covariation_analysis(
    Radius=0,
    output_niche_prediction_dir=None,
    refpath="./inputRef/",
    quepath="./inputQuery/",
    ref_cluster_tag="cluster",
    ref_original_counts="Original_counts.h5ad",
    LRdbFilename="./utils/NiCoLRdb.txt",
    iNMFmode=True,
    no_of_factors=3,
    shap_analysis=False,
    ...
    for radius in inputRadius:
        ...
        (
            ...
            annotation_spatial_barcode_id,
            annotation_spatial_cluster_id,
            spatialcell_unique_clustername,
            spatialcell_unique_clusterid,
        ) = read_spatial_data(clusterFilename, celltypeFilename)

        neighbors = pickle.load(
            open(niche_prediction_dir + "neighbors_" + str(radius) + ".p", "rb")
        )
        distances = pickle.load(
            open(niche_prediction_dir + "distances_" + str(radius) + ".p", "rb")
        )

        covariation_dir = (
            covariation_outdir + "R" + str(radius) + "_F" + str(no_of_factors) + "/"
        )
        ...
        fname = (
            niche_prediction_dir
            + strategy
            + "/classifier_matrices_"
            + str(radius)
            + ".npz"
        )
        ...
        logistic_predicted_interactions = find_logistic_regression_interacting_score(
            logistic_cmn,
            logistic_coef,
            logistic_CTFeatures,
            nameOfCellType,
            logistic_coef_cutoff,
        )
        ...
        inputdata["annotation_singlecell_celltypename"] = (
        ...
        inputdata["cutoff_to_count_exp_cell_population"] = (
            cutoff_to_count_exp_cell_population
        )
        inputdata["LRdb"] = LRdb
        ...
        if flag == 1:
            (
                ...
                save_scFactors,
                save_spFactors,
            ) = compute_PC_space(input, full_ad_sc)
            ...
            pickle.dump(
                ...
                open(covariation_dir + "factors_info.p", "wb"),
            )
...
def plot_cosine_and_spearman_correlation_to_factors(
    input,
    choose_celltypes=[],
    NOG_Fa=30,
    saveas="pdf",
    ...
    PCA_of_sc_cluster_accordingto_spatial_clusterid, save_scFactors, save_spFactors = (
        pickle.load(open(input.covariation_dir + "factors_info.p", "rb"))
    )
    ...
    for fi in perform:
        ...
        (
            ...
            cosine_factors,
            alpha,
        ) = PCA_of_sc_cluster_accordingto_spatial_clusterid[clid]
        CC_celltype_name = input.spatialcell_unique_clustername[fi]

        genename_full = CC_gene
        # sc_cosine=find_correlation_bw_genes_and_PC_component_in_singlecell_cosine(H.T,CbyG)
        gname2b, geneNMF2b = top_genes_in_correlation_list_without(
            genename_full, cosine_factors, NOG_Fa
        )
        # sc_spearman=find_correlation_bw_genes_and_PC_component_in_singlecell(H.T,CbyG)
        gname3b, geneNMF3b = top_genes_in_correlation_list_without(
            genename_full, spearman_factors, NOG_Fa
        )
        ...
        gs = fig.add_gridspec(
            ncols=4, nrows=1, wspace=0.5, width_ratios=[0.5, 2, 2, 0.5]
        )
        ...
        b = snn.heatmap(
            selectedGenesAvgExp_cosine, yticklabels=gname2b, ax=ax0
        )  # componentlabel,ax=ax
        ...
        b = snn.heatmap(
            selectedGenesAvgExp, yticklabels=gname3b, ax=ax3
        )  # componentlabel,ax=ax
        ...
        print(
            ...
            input.nmf_output
            + remove_extra_character_from_name(CC_celltype_name)
            + "."
            + saveas,
        )
        fig.savefig(
...
def plot_feature_matrices(
    input, showit=True, saveas="pdf", transparent_mode=False, dpi=300, figsize=(10, 10)
):
    """
...
def plot_significant_regression_covariations_as_circleplot(
    input,
    choose_celltypes=[],
    saveas="pdf",
    pvalue_cutoff=0.05,
    ...
    print(
        "The regression figures as pvalue circle plots are saved in following path ",
        input.regression_outdir + "pvalue_coeff_circleplot_*",
    )

    for i in perform:
        filename = input.spatialcell_unique_clustername[i]
        temp = np.where(
            input.spatialcell_unique_clusterid[i] == input.annotation_spatial_cluster_id
        )
        index = temp[0]
        ...
        for k in range(len(xlabel)):
            for j in range(input.no_of_pc):
                ylabelname.append(
                    xlabel[k] + "_s" + "%0.3f" % score[k] + "_Fa" + str(j + 1)
                )
        ...
        ax.set(
            xticks=np.arange(M),
            yticks=np.arange(N),
            xticklabels=ylabelname,
            yticklabels=componentlabel,
        )
        ax.set_xticks(np.arange(M + 1) - 0.5, minor=True)
        ax.set_yticks(np.arange(N + 1) - 0.5, minor=True)
        ax.set_xticklabels(ylabelname, size=8, rotation=90)
        ax.set_title(
            filename + r",$\alpha$=" + str(alpha) + ",EVS=" + percentVE, fontsize=6
        )
        ax.grid(which="minor")
        ...
        savefname = remove_extra_character_from_name(
            str(input.spatialcell_unique_clusterid[i]) + "_" + filename
        )
        ...
        fig.savefig(
            input.regression_outdir
            + "pvalue_coeff_circleplot_"
            + savefname
            + "."
            + saveas,
            bbox_inches="tight",
...
def plot_significant_regression_covariations_as_heatmap(
    input,
    choose_celltypes=[],
    saveas="pdf",
    transparent_mode=False,
    ...
    print(
        "The regression figures as pvalue heatmap plots are saved in following path ",
        input.regression_outdir + "pvalue_coeff_heatmap_*",
    )
...
def save_LR_interactions_in_excelsheet_and_regression_summary_in_textfile_for_interacting_cell_types(
    input,
    pvalueCutoff=0.05,
    ...
    workbook = xlsxwriter.Workbook(
        input.covariation_dir + "Lig_and_Rec_enrichment_in_interacting_celltypes.xlsx"
    )
    ...
    for i in range(n):
        ...
        for k in range(normalized_ridge_coef.shape[0]):
            ...
            for j in range(normalized_ridge_coef.shape[1]):
                ...
                if (pvalue[k, j] < pvalueCutoff) & (
                    abs(normalized_ridge_coef[k, j]) > coeff_cutoff_for_rid_reg
                ):
                    # if True:
                    if score[index] > coeff_cutoff_for_log_reg:
                        ...
                        common_genes = list(
                            set(top_genes_in_CC).intersection(set(top_genes_in_NC))
                        )

                        if CC_celltype_name != NC_celltype_name[index]:
                            for ele in range(len(Found1)):
                                header = [
                                    str(i) + "-" + str(interaction_id),
                                    CC_celltype_name + "(cc)",
                                    NC_celltype_name[index] + "(nc)",
                                    score[index],
                                    k + 1,
                                    1 + pc_index_nc[j],
                                    normalized_ridge_coef[k, j],
                                    "Ligand(A)",
                                    "Receptor(B)",
                                    "GeneCor(Lig)",
                                    "GeneCor(Rec)",
                                ...
                                for ri in range(15):
                                    ...
                                    worksheet_local.write(
                                        worksheetrow_local, ri, header[ri]
                                    )
                                worksheetrow += 1
                                worksheetrow_local += 1
                        ...
                        fout.write(
                            ...
                            + str(1 + pc_index_nc[j])
                            + "\t"
                            + NC_celltype_name[index]
                            + "\tRegCoeff=%0.3f" % (normalized_ridge_coef[k, j])
                            + "\t"
                            + "pvalue=%0.2e" % pvalue[k, j]
                            + "\t-log10(pvalue)=%0.2f" % (-np.log10(pvalue[k, j]))
                        )  # str(interaction_id)
                        fout.write("\n")
...
def find_LR_interactions_in_interacting_cell_types(
    input,
    ...
    saveLRplotsSecond = (
        input.covariation_dir + "Plot_ligand_receptor_in_niche_nc_vs_cc/"
    )
                ...
                if (pvalue[k, j] < pvalueCutoff) & (
                    ...
                    if score[index] > coeff_cutoff_for_log_reg:
                        ...
                        if len(choose_interacting_celltype_pair) > 1:
                            if (
                                NC_celltype_name[index]
                                == choose_interacting_celltype_pair[1]
                            ):
                                ncflag = 1
                            else:
                                ncflag = 0
...
def make_excel_sheet_for_gene_correlation(input):
    ...
    for i in range(input.no_of_pc):
        worksheetFullGene_spearman.append(
            workbook.add_worksheet("spearman scRNAseq Fa" + str(i + 1))
        )
        worksheetFullGene_cosine.append(
            workbook.add_worksheet("cosine scRNAseq Fa" + str(i + 1))
        )

    worksheetSpatialGene_spearman = []
    worksheetSpatialGene_cosine = []
    for i in range(input.no_of_pc):
        worksheetSpatialGene_spearman.append(
            workbook.add_worksheet("spearman spatial Fa" + str(i + 1))
        )
        worksheetSpatialGene_cosine.append(
            workbook.add_worksheet("cosine spatial Fa" + str(i + 1))
        )
    ...
    for i in range(n):
        ...
        worksheetrow = 0
        worksheetAvgGeneExp.write(
            worksheetrow, 3 * i, input.spatialcell_unique_clustername[i]
        )
        for j in range(input.no_of_pc):
            worksheetFullGene_spearman[j].write(
                worksheetrow,
                (input.no_of_pc + 2) * i,
                input.spatialcell_unique_clustername[i],
            )
            worksheetSpatialGene_spearman[j].write(
                worksheetrow,
                (input.no_of_pc + 2) * i,
                input.spatialcell_unique_clustername[i],
            )
            worksheetFullGene_cosine[j].write(
                worksheetrow,
                (input.no_of_pc + 2) * i,
                input.spatialcell_unique_clustername[i],
            )
            worksheetSpatialGene_cosine[j].write(
                worksheetrow,
                (input.no_of_pc + 2) * i,
                input.spatialcell_unique_clustername[i],
            )
        ...
        headersave_full, headersave_common, sort_full, sort_common = (
            sorting_of_factors_for_showing_the_value_in_excelsheet(
                CC_corr_spearman, input.no_of_pc, gene, genenames
            )
        )
        ...
        for k in range(input.no_of_pc):
            ...
            for rj in range(len(indsort)):
                ...
                for ri in range(len(header)):
                    worksheetFullGene_spearman[k].write(
                        worksheetrow,
                        (input.no_of_pc + 2) * i + ri,
                        header[ri],
                        genecolor,
                    )
                worksheetrow += 1
            ...
            for rj in range(len(indsort)):
                ...
                for ri in range(len(header)):
                    worksheetSpatialGene_spearman[k].write(
                        worksheetrow, (input.no_of_pc + 2) * i + ri, header[ri]
                    )
                worksheetrow += 1

        headersave_full, headersave_common, sort_full, sort_common = (
            sorting_of_factors_for_showing_the_value_in_excelsheet(
                CC_corr_cosine, input.no_of_pc, gene, genenames
            )
        )
        for k in range(input.no_of_pc):
            worksheetrow = fixvalue
            ...
            for rj in range(len(indsort)):
                ...
                for ri in range(len(header)):
                    worksheetFullGene_cosine[k].write(
                        worksheetrow,
                        (input.no_of_pc + 2) * i + ri,
                        header[ri],
                        genecolor,
            ...
            for rj in range(len(indsort)):
                header = headersave_common[indsort[rj]]
                for ri in range(len(header)):
                    worksheetSpatialGene_cosine[k].write(
                        worksheetrow, (input.no_of_pc + 2) * i + ri, header[ri]
                    )
                worksheetrow += 1
    workbook.close()
...
def pathway_analysis(
...
):  # background_geneName,background_expression
    """
    ...
    for fi in perform:
        ...
        for j in range(input.no_of_pc):
            ...
            sname1 = (
                tname + "Fa" + str(j + 1) + "_" + ccname + "_c" + str(int(100 * cutoff))
            )
            ...
            if flag == 1:
                for i in range(len(database)):
                    titlename1 = (
                        titlename + "[" + database[i] + "]" + " #G=" + str(len(ga1))
                    )
                    sname2 = sname1 + "_" + database[i]
                    barplotsavename = savename + sname2
                    finalsavename = savename + sname2 + "." + saveas

                    enr_res1 = gseapy.enrichr(
                        gene_list=ga1,
                        organism=pathwayorganism,
                        gene_sets=database[i],
                        cutoff=pvalue_cutoff_enrichr,
                    )

                    data = enr_res1.res2d.loc[
                        enr_res1.res2d[object_for_sorting] < pvalue_cutoff
                    ].copy()

                    object_for_color_key = object_for_color
                    object_for_xaxis_key = object_for_xaxis

                    if (object_for_color == "Adjusted P-value") | (
                        object_for_color == "P-value"
                    ):
                        columntag = "-log10(" + object_for_color + ")"
                        data[columntag] = -np.log10(data[object_for_color])
                        data[columntag] = data[columntag].apply(
                            lambda x: round(x, barplot_log10_pvalue_roundoff)
                        )
                        if display_plot_as == "barplot":
                            object_for_color_key = columntag

                    if (object_for_xaxis == "Adjusted P-value") | (
                        object_for_xaxis == "P-value"
                    ):
                    ...
                    data = data.sort_values(
                        by=object_for_xaxis_key, ascending=barplot_ascending_order
                    )
                    ...
                    try:
                        if display_plot_as == "barplot":
                            ...
                            colors = plt.colormaps.get_cmap(input_colormap)(
                                data[object_for_color_key]
                                / data[object_for_color_key].max()
                            )  # Normalize for colormap
                            bars = ax.barh(
                                data[object_for_yaxis],
                                data[object_for_xaxis_key],
                                color=colors,
                                edgecolor=barplot_edgecolor,
                                linewidth=barplot_linewidth,
                            )
                            ...
                            sm = plt.cm.ScalarMappable(
                                cmap=input_colormap,
                                norm=plt.Normalize(
                                    vmin=data[object_for_color_key].min(),
                                    vmax=data[object_for_color_key].max(),
                                ),
                            )
                            sm.set_array([])
                            cbar = fig.colorbar(
                                sm, ax=ax, shrink=barplot_colorbar_length_shrink
                            )  # Shrink the colorbar by 70%
                            cbar.set_label(
                                object_for_color_key, rotation=270, labelpad=15
                            )
                        ...
                        if display_plot_as == "dotplot":
                            if savefigure:
                                gseapy.dotplot(
                                    enr_res1.res2d,
                                    title=titlename1,
                                    ofname=finalsavename,
                                    fontsize=fontsize,
                                    size=circlesize,
                                    cmap=plt.cm.get_cmap(input_colormap),
                                    column=object_for_color,
                                    x=object_for_xaxis,
                                    y=object_for_yaxis,
                                    x_order=dotplot_x_order,
                                    y_order=dotplot_y_order,
                                    cutoff=pvalue_cutoff,
                                    top_term=top_term,
                                    figsize=figsize,
                                    xticklabels_rot=dotplot_xticklabels_rot,
                                    yticklabels_rot=dotplot_yticklabels_rot,
                                    marker=dotplot_marker,
                                    show_ring=dotplot_show_ring,
                                )
                            else:
                                gseapy.dotplot(
                                    enr_res1.res2d,
                        ...
                        Exception
                    ) as e:  # Exception: Error getting the Enrichr libraries
                        pass


def extract_and_plot_top_genes_from_chosen_factor_in_celltype(
    input,
    choose_celltype,
    choose_factor_id,
    top_NOG=30,
    ...
    if flag_correct == 1:
        for fi in perform:
            ...
            if True:
                ...
                if positively_correlated:
                    ...
                    d = {
                        "Gene": comgene,
                        "Fa": vp1,
                        "mean_expression": pos_avg1,
                        "proportion_of_population_expressed": pos_pop1,
                    }
                    ...
                    d = {
                        "Gene": comgene,
                        "Fa": vn1,
                        "mean_expression": neg_avg1,
                        "proportion_of_population_expressed": neg_pop1,
                    }
                    df = pd.DataFrame(data=d)
                    df.set_index("Gene")
            ...
            legend2 = ax[0].legend(
                *p0.legend_elements(**kw),
                loc="lower center",
                bbox_to_anchor=(0.25, -0.25),
                title="Fraction of cells expressed",
                frameon=False,
            )  #
...
def find_correlation_bw_genes_and_PC_component_in_singlecell(
    KcomponentCluster, clusterExpression
):
    """
    ...
    mat = np.zeros(
        (clusterExpression.shape[1], KcomponentCluster.shape[1]), dtype=float
    )
...
def find_correlation_bw_genes_and_PC_component_in_singlecell_cosine(
    KcomponentCluster, clusterExpression
):
    """
...
def find_PC_of_invidualCluster_in_SC(
    seed,
    spatial_integration_modality,
    scbarcode,
    iNMFmode,
    scadata,
    no_of_pc,
    ...
    sc_cosine = find_correlation_bw_genes_and_PC_component_in_singlecell_cosine(
        H.T, CbyG
    )
    sc_spearman = find_correlation_bw_genes_and_PC_component_in_singlecell(H.T, CbyG)
    ...
    sc_cluster_exp_more_than_threshold = np.sum(
        sc_cluster_exp_more_than_threshold, axis=0
    )
    sc_cluster_exp_more_than_threshold = (
        sc_cluster_exp_more_than_threshold / CbyG.shape[0]
    )
    ...
    for j in range(len(input.neighbors)):
        ...
        for k in range(len(input.neighbors[j])):
            ...
            if NC_cluster_id not in temp:
                ...
                temp[NC_cluster_id] = np.concatenate(
                    (temp[NC_cluster_id], factor * PC_component_of_NC)
                )
...
def compute_PC_space(input, sct_ad_sc_full):
    ...
    print(
        "\nThe spatial cluster name does not match the scRNA-seq cluster name ",
        b - common,
    )
    print(
    ...
    if flag == 1:
        ...
        for i in range(n):
            ...
            for j in range(len(input.singlecell_unique_clustername)):
                if (
                    input.singlecell_unique_clustername[j]
                    == input.spatialcell_unique_clustername[i]
                ):
            ...
            (
                ...
                sc_cluster_exp_more_than_threshold,
                alpha,
            ) = find_PC_of_invidualCluster_in_SC(
                seed,
                input.spatial_integration_modality,
                scbarcode,
                input.iNMFmode,
                input.ad_sc,
                input.no_of_pc,
                ...
                sct_ad_sc_full,
                input.spatialcell_unique_clustername[i],
                input.cutoff_to_count_exp_cell_population,
            )

            PCA_of_sc_cluster_accordingto_spatial_clusterid[clidsp] = [
                sc_spearman,
                pc_sp,
                sc_genenames,
                sc_cluster_mean_exp,
...
def model_linear_regression(input, logistic_predicted_interactions):
    ...
    for i in range(len(input.spatialcell_unique_clusterid)):
        ...
        positive_interacted_CT = logistic_predicted_interactions[
            input.spatialcell_unique_clustername[i]
        ]
        newindex = []
        xlabel = []
        ...
        for j in range(len(input.spatialcell_unique_clustername)):
            ...
            for k in range(len(positive_interacted_CT)):
                if (
                    positive_interacted_CT[k][0]
                    == input.spatialcell_unique_clustername[j]
                ):
        ...
        saveoutname = (
            str(input.spatialcell_unique_clusterid[i])
            + "_"
            + input.spatialcell_unique_clustername[i]
        )
        (
        ...
        save_coef[input.spatialcell_unique_clusterid[i]] = [
            coef,
            intercept,
            alpha,
            xlabel,
...
def run_ridge_regression(
    input, saveoutname, ylabelname, target, neighborhoodClass, shap_cluster_cutoff
):
    """
    ...
    for i in range(y_train.shape[1]):
        ...
        if detM > 0:
            ...
            p_values1 = np.array(
                [[2 * (1 - scipy.stats.t.cdf(np.abs(ii), df - 1)) for ii in ts_b]]
            )
            pv[i] = p_values1[:, 1:]
            # if flag==1:
    ...
    if input.shap_analysis:
        ...
        explainer = shap.explainers.Linear(
            LR, x_train, feature_names=ylabelname, feature_perturbation="interventional"
        )
        ...
        for i in range(y_train.shape[1]):
            ...
            shap.plots.bar(
                shap_values,
                clustering=clust,
                clustering_cutoff=shap_cluster_cutoff,
                show=False,
            )
            plt.title(
                "True to the model "
            ...
            plt.savefig(
                dir1 + saveoutname + "_Fa" + str(i + 1) + ".png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close("all")
            ...
            plt.savefig(
                dir2 + saveoutname + "_Fa" + str(i + 1) + ".png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close("all")
...
def find_logistic_regression_interacting_score(
    cmn, coef, CTFeatures, nameOfCellType, logistic_coef_cutoff
):
    """
    ...
    for i in range(len(InteractingCTs)):
        ...
        for j in range(len(nCT)):
            if cCT not in logistic_predicted_interactions:
                ...
                logistic_predicted_interactions[cCT].append(
                    [nCT[j], Interacting_score[j]]
                )
            ...
            if gene[ind1[i]].upper() in listofallLR:
                cc_genes2.append(
                    [
                        gene[ind1[i]],
                        "%0.2f" % first[ind1[i]],
                        CC_meanExpression[ind1[i]],
                        CC_popExpression[ind1[i]],
                    ]
                )
            ...
            if gene[ind2[i]].upper() in listofallLR:
                nc_genes2.append(
                    [
                        gene[ind2[i]],
                        "%0.2f" % second[ind2[i]],
                        NC_meanExpression[ind2[i]],
                        NC_popExpression[ind2[i]],
                    ]
                )
...
def sorting_of_factors_for_showing_the_value_in_excelsheet(
    CC_corr, no_of_pc, gene, genenames
):
    """
...
def plot_ligand_receptor_in_interacting_celltypes(
    CC_celltype_name,
    NC_celltype_name,
    logRegScore,
    pc1,
    ...
    if flag == "First":
        for ele in range(len(Found1)):
            ...
            if (ligExpCellPop > LR_plot_Exp_thres) & (
                recExpCellPop > LR_plot_Exp_thres
            ):
                ...
                A.append(
                    CC_celltype_name + "(cc)_Fa" + str(pc1) + "_" + Found1[ele][0][0]
                )
                B.append(
                    NC_celltype_name + "(nc)_Fa" + str(pc2) + "_" + Found1[ele][1][0]
                )
    ...
    if flag == "Second":
        for ele in range(len(Found2)):
            ...
            if (ligExpCellPop > LR_plot_Exp_thres) & (
                ...
                A.append(
                    NC_celltype_name + "(nc)_Fa" + str(pc2) + "_" + Found2[ele][0][0]
                )
                B.append(
                    CC_celltype_name + "(cc)_Fa" + str(pc1) + "_" + Found2[ele][1][0]
                )
    if flag == "Both":
        for ele in range(len(Found1)):
    ...
    if (len(A) > 0) & (len(B) > 0):
        ...
        values = [
            df_piv[dir] for dir in ["north", "east", "south", "west"]
        ...
        norms = [
            plt.Normalize(fmin, fmax),
            plt.Normalize(pmin, pmax),
            plt.Normalize(fmin, fmax),
            plt.Normalize(pmin, pmax),
        ]
        ...
        imgs = [
            ax.tripcolor(t, np.ravel(val), norm=norm, cmap=cmap, ec="white")  # norm=[]
        ...
        savefname = (
            ...
            + str(pc1)
            + "_"
            + remove_extra_character_from_name(NC_celltype_name)
            + "_Fa"
            + str(pc2)
        )

        fig.savefig(
            saveLRplots + savefname + "." + saveas,
            bbox_inches="tight",
            transparent=transparent_mode,
            dpi=dpi,
        )
...
def visualize_factors_in_scRNAseq_umap(
    input,
    choose_interacting_celltype_pair,
    ...
    if len(visualize_factors_id) == 1:
        ...
        plot_all_ct(
            ...
            cellsinCT,
            ax,
            [choose_interacting_celltype_pair[0]],
            v1,
            cmap,
            ms,
            msna,
        ...
        ax.set_axis_off()
        ax.set_title(
            choose_interacting_celltype_pair[0] + ":Fa" + str(visualize_factors_id[0])
        )
    ...
    if len(visualize_factors_id) == 2:
        ...
        ax[1].set_title(
            choose_interacting_celltype_pair[1] + ":Fa" + str(visualize_factors_id[1])
        )

    fig.tight_layout()
    print(
        "The figures are saved: ",
        input.covariation_dir + "scRNAseq_factors_in_umap." + saveas,
    )
    fig.savefig(
        input.covariation_dir + "scRNAseq_factors_in_umap." + saveas,
        bbox_inches="tight",
...
def plot_all_ct(CTname, PP, cellsinCT, ax, mycelltype, Fa, cmap, ms, msna):
    ...
    for i in range(len(CTname)):
        ...
        if CTname[i] in mycelltype:
            ...
            p1 = ax.scatter(
                PP[index, 0], PP[index, 1], s=ms, c=Fa[index], marker="o", cmap=cmap
            )
        else:
            naindex = naindex + index

    ax.plot(
        PP[naindex, 0], PP[naindex, 1], ".", color="0.5", label="NA", markersize=msna
    )
    plt.colorbar(p1, ax=ax, shrink=0.5)


def visualize_factors_in_spatial_umap(
    input,
    choose_interacting_celltype_pair,
    visualize_factors_id,
    umap_tag="X_umap",
    ...
    cellname = np.reshape(
        input.annotation_spatial_barcode_id,
        (len(input.annotation_spatial_barcode_id), 1),
    )
    umap_data = sort_index_in_right_order(cellname, umap_not_order)
    PCA_of_sc_cluster_accordingto_spatial_clusterid, save_scFactors, save_spFactors = (
        pickle.load(open(input.covariation_dir + "factors_info.p", "rb"))
    ...
    print(
        "The figures are saved: ",
        input.covariation_dir + "spatial_factors_in_umap." + saveas,
    )
    fig.savefig(
        input.covariation_dir + "spatial_factors_in_umap." + saveas,
        bbox_inches="tight",
...
def plot_top_genes_for_a_given_celltype_from_all_factors(
    input,
    choose_celltypes=[],
    top_NOG=20,
    rps_rpl_mt_genes_included=True,
    ...
    for fi in perform:
        ...
        for i in range(6):
            ...
            p0 = ax[0, i].scatter(
                x, y, s=bigs, marker="o", c=z, cmap="RdBu_r"
            )  #'cm.cmap_name
            ...
            legend2 = ax[1, i].legend(
                *p1.legend_elements(**kw),
                loc="upper right",
                title="Fraction of \ncells \nexpressed",
...
def plot_top_genes_for_pair_of_celltypes_from_two_chosen_factors(
    input,
    choose_interacting_celltype_pair,
    visualize_factors_id,
    top_NOG=20,
    ...
    for fi in range(n):
        ...
        if CC_celltype_name == choose_interacting_celltype_pair[0]:
            ...
            ga1, va1 = find_interest_of_genes(
                z1, pop1, mu1, rps_rpl_mt_genes_included, CC_gene, top_NOG
            )
    ...
    for fi in range(n):
        ...
        if CC_celltype_name == choose_interacting_celltype_pair[1]:
            ...
            if correlation_with_spearman:
                z2 = spearman_factors[
                    :, visualize_factors_id[1] - 1
                ]  # np.hstack((z,spearman_factors))
            else:
                z2 = cosine_factors[
                    :, visualize_factors_id[1] - 1
                ]  # np.hstack((z,cosine_factors))
            ga2, va2 = find_interest_of_genes(
                z2, pop2, mu2, rps_rpl_mt_genes_included, CC_gene, top_NOG
            )
    ...
    xlabels11 = [
        choose_interacting_celltype_pair[0].replace("_", " ") + ":avg exp",
        choose_interacting_celltype_pair[0].replace("_", " ")
        + ":Fa"
        + str(visualize_factors_id[0]),
        choose_interacting_celltype_pair[1].replace("_", " ") + ":avg exp",
        choose_interacting_celltype_pair[1].replace("_", " ")
        + ":Fa"
        + str(visualize_factors_id[1]),
    ]
    xlabels = [0, 1]
    xlabels[0] = [
        choose_interacting_celltype_pair[0].replace("_", " ")
    ...
    savename = (
        remove_extra_character_from_name(choose_interacting_celltype_pair[0])
        + "_"
        + remove_extra_character_from_name(choose_interacting_celltype_pair[1])
    )
    print("The figures are saved: ", savefigdir + "combined_" + savename + "." + saveas)
    plt.savefig(
        savefigdir + "combined_" + savename + "." + saveas,
        bbox_inches="tight",
        transparent=transparent_mode,
        dpi=dpi,
    )
...
def find_interest_of_genes(
    source, pop, mu, rps_rpl_mt_genes_included, CC_gene, top_NOG
):
    """
