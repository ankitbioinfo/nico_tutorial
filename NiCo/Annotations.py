...
def find_mutual_nn(minkowski_order, data1, data2, sp_barcode, sc_barcode, k1, k2):
    ...
    n_jobs = -1
    d1, k_index_1 = cKDTree(data1).query(
        x=data2, k=k1, p=minkowski_order, workers=n_jobs
    )
    d2, k_index_2 = cKDTree(data2).query(
        x=data1, k=k2, p=minkowski_order, workers=n_jobs
    )
...
def find_commnon_MNN(input):
    ...
    index_annot_sc = find_annotation_index(
        input.annotation_singlecell_barcode_id, mnn_singlecell_matchpair_barcode_id
    )
    index_annot_sp = find_annotation_index(
        input.annotation_spatial_barcode_id, mnn_spatial_matchpair_barcode_id
    )
    ...
    for i in range(len(input.annotation_singlecell_cluster_id)):
        d_single[input.annotation_singlecell_barcode_id[i]] = (
            input.annotation_singlecell_cluster_id[i]
        )
    ...
    for i in range(len(input.annotation_spatial_cluster_id)):
        d_spatial[input.annotation_spatial_barcode_id[i]] = (
            input.annotation_spatial_cluster_id[i]
        )
...
def visualize_spatial_anchored_cell_mapped_to_scRNAseq(
    input, saveas="pdf", transparent_mode=False, showit=True, figsize=(12, 10)
):
    """
    ...
    plt.savefig(
        input.output_annot + "visualize_anchors." + saveas,
        bbox_inches="tight",
...
def find_anchor_cells_between_ref_and_query(
    ...
    if os.path.isfile(fmnn):
        ...
        #    flag=0
    if flag == 1:
        input_sp, input_sc, sp_barcode, sc_barcode = (
            sct_return_sc_sp_in_shared_common_PC_space(
                ad_sp_ori, ad_sc_ori, no_of_pc, method
            )
        )
        # print('sp',input_sp.shape,'\nsc',input_sc.shape)
        corrected = find_mutual_nn(
            minkowski_order,
            input_sp,
            input_sc,
            sp_barcode,
        ...
        n_jobs = -1
        k_dist, k_index = cKDTree(input_sp).query(
            x=input_sp, k=neigh, p=minkowski_order, workers=n_jobs
        )
        knn_neigh = []
...
def nico_based_annotation(
    ...
    **This function performs NiCo-based annotation of spatial cell transcriptomes by using label transfer from scRNAseq data.**
    ...
    df = previous.adata_query.obs[
        guiding_spatial_cluster_resolution_tag
    ]  # .to_csv(spatialclusterFilename,header=True)
    annotation_spatial_barcode_id = df.index.to_numpy()
    annotation_spatial_cluster_id = df.to_numpy()
    spatialcell_unique_clustername = []
    spatialcell_unique_clusterid = sorted(
        list(np.unique(annotation_spatial_cluster_id))
    )
    d = {}
    ...
    spatial_deg_annotation_output_clustername = (
        spatial_annotation_output_fname + "_cluster.csv"
    )
    spatial_deg_annotation_output_celltypename = (
        spatial_annotation_output_fname + "_ct_name.csv"
    )
    ...
    inputvar["spatial_deg_annotation_output_clustername"] = (
        spatial_deg_annotation_output_clustername
    )
    inputvar["spatial_deg_annotation_output_celltypename"] = (
        spatial_deg_annotation_output_celltypename
    )
    inputvar["across_spatial_clusters_dispersion_cutoff"] = (
        across_spatial_clusters_dispersion_cutoff
    )
    inputvar["number_of_iteration_to_perform_celltype_annotations"] = (
        number_of_iteration_to_perform_celltype_annotations
    )
    inputvar["resolved_tie_issue_with_weighted_nearest_neighbor"] = (
        resolved_tie_issue_with_weighted_nearest_neighbor
    )
    ...
    a = np.reshape(
        input.annotation_singlecell_barcode_id,
        (len(input.annotation_singlecell_barcode_id), 1),
    )
    b = np.reshape(
        input.annotation_singlecell_cluster_id,
        (len(input.annotation_singlecell_cluster_id), 1),
    )

    sc_clusters = np.hstack((a, b))
    sp_cell_identity = find_all_the_spatial_cells_mapped_to_single_cells(
        sc_ctype_id, sc_clusters, mnn, sc_ctype_name
    )
    ...
    if (input.resolved_tie_issue_with_weighted_nearest_neighbor) == "No":
        all_anchored_mapped = (
            resolved_confused_and_unmapped_mapping_of_cells_with_majority_vote(
                confused, G, all_mapped, unique_mapped, resolutionClusterWise
            )
        )
    else:
        all_anchored_mapped = resolved_confused_and_unmapped_mapping_of_cells_with_weighted_average_of_inverse_distance_in_neighbors(
            confused, G, weights, all_mapped, unique_mapped, resolutionClusterWise
        )
    ...
    for iter in range(input.number_of_iteration_to_perform_celltype_annotations):
        unmapped_cellname, unmapped_deg = find_unmapped_cells_and_deg(
            deg, availabled_anchors_mapped
        )
        ...
        if (input.resolved_tie_issue_with_weighted_nearest_neighbor) == "No":
            ...
            unique_mapped = resolved_confused_and_unmapped_mapping_of_cells_with_weighted_average_of_inverse_distance_in_neighbors(
                unmapped_cellname,
                G,
                weights,
                availabled_anchors_mapped,
        ...
        deg_annot_cluster_fname = (
            input.output_annot
            + str(iter + 1)
            + "_"
            + input.spatial_deg_annotation_output_clustername
        )
        deg_annot_ct_fname = (
            input.output_annot
            + str(iter + 1)
            + "_"
            + input.spatial_deg_annotation_output_celltypename
        )

        nico_cluster = write_annotation(
            deg_annot_cluster_fname, deg_annot_ct_fname, unique_mapped, cellname
        )
...
def find_all_the_spatial_cells_mapped_to_single_cells(
    sc_ctype_id, sc_clusters, mnn, sc_ctype_name
):
    """
...
def write_annotation(
    ...
    for i in range(len(sc_ctype_name)):
        fw.write(
            str(i) + "," + sc_ctype_name[i] + "," + str(d2[sc_ctype_name[i]]) + "\n"
        )
        d[sc_ctype_name[i]] = i
    fw.close()
...
def resolved_confused_and_unmapped_mapping_of_cells_with_weighted_average_of_inverse_distance_in_neighbors(
    confused,
    G,
    weights,
    all_mapped,
    unique_mapped,
    sp_leiden_barcode2cluid_resolution_wise,
):
    """
        ...
        if True:
            ...
            if (finalone == "NM") & (len(t1) > 0):
                ...
                if len(t1) == 1:
                    finalone = t2[
                        0
                    ]  # If neighbors belong to only one cell type then it assigned to that
                elif (
                    t1[index[0]] >= t1[index[1]]
                ):  # If neighbors belong to many cell type (the first one has largest degree)
                    if (
                        t3[index[0]] < t3[index[1]]
                    ):  # Here it preference for the normalized inverse distance score
                        finalone = t2[
                            index[1]
                        ]  # if the second cell type has lower degree than first but it has higher weight score then it choses the second cell type
                    else:
                        finalone = t2[
                            index[0]
                        ]  # if the second cell type has lower degree than first but it has lower weight score then it choses the first cell type
...
def resolved_confused_and_unmapped_mapping_of_cells_with_majority_vote(
    confused, G, all_mapped, unique_mapped, sp_leiden_barcode2cluid
):
    """
        ...
        if True:
            ...
            for i in range(len(high2low)):
                if high2low[i].find("_a#d_") == -1:
                    t1.append(
                        c[high2low[i]]
                    )  # integer number to denote the degree for the negihboring cells who has similar cell type
...
def visualize_umap_and_cell_coordinates_with_all_celltypes(
    output_annotation_dir=None,
    output_nico_dir=None,
    ...
    print(
        ...
        fig_save_path + "tissue_and_umap_with_all_celltype_annotations." + saveas,
    )
    fig.savefig(
        fig_save_path + "tissue_and_umap_with_all_celltype_annotations." + saveas,
        bbox_inches="tight",
...
def save_annotations_in_spatial_object(
    inputdict, anndata_object_name="nico_celltype_annotation.h5ad"
):
    """
...
def visualize_umap_and_cell_coordinates_with_selected_celltypes(
    ...
    for fi in range(len(mycluster_interest_all)):
        ...
        plot_specific_ct(
            mycluster_interest, umap_data[:, [1, 2]], id2, ax[1], cmap, ms, msna
        )
        ...
        filename = (
            fig_save_path
            + remove_extra_character_from_name(mycluster_interest[0])
            + str(fi)
        )
...
def plot_all_ct(CTname, PP, cellsinCT, ax, flag, cmap):
    ...
    for i in range(len(CTname)):
        ...
        rgba = cmap(cumsum[i])
        ax.plot(
            PP[index, 0], PP[index, 1], "o", label=labelname, color=rgba, markersize=1
        )
...
def plot_specific_ct(CTname, PP, index, ax, cmap, ms, msna):
    ...
    ax.plot(
        PP[remaining_index, 0],
        PP[remaining_index, 1],
        ".",
        color="0.5",
        label="NA",
        markersize=msna,
