...
def create_spatial_CT_feature_matrix(
    radius, PP, louvain, noct, fraction_CT, saveSpatial, epsilonThreshold
):
    """
    ...
    np.savez(
        saveSpatial + "normalized_spatial_neighborhood_" + str(radius) + ".npz",
        input_mat_for_log_reg=input_mat_for_log_reg,
    )
...
def plot_confusion_matrix(
    input, saveas="pdf", showit=True, transparent_mode=False, dpi=300, figsize=(5.5, 5)
):
    """
    ...
    print(
        ...
        input.niche_pred_outdir + "Confusing_matrix_" + filename + "." + saveas,
    )
    plt.savefig(
        input.niche_pred_outdir + "Confusing_matrix_" + filename + "." + saveas,
        bbox_inches="tight",
...
def plot_coefficient_matrix(
    input, saveas="pdf", showit=True, transparent_mode=False, dpi=300, figsize=(5, 8)
):
    """
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir + "weight_matrix_" + filename + "." + saveas,
    )
    plt.savefig(
        input.niche_pred_outdir + "weight_matrix_" + filename + "." + saveas,
        bbox_inches="tight",
...
def plot_predicted_probabilities(
    input, saveas="pdf", showit=True, transparent_mode=False, dpi=300, figsize=(12, 6)
):
    """
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir + "predicted_probability_" + filename + "." + saveas,
    )
    plt.savefig(
        input.niche_pred_outdir + "predicted_probability_" + filename + "." + saveas,
        bbox_inches="tight",
...
def plot_roc_results(
    ...
    for i in range(nrows * ncols):
        ...
        ax[value[0], value[1]].set_title(
            str(highestROCofcelltype[i])
            + " : "
            + input.nameOfCellType[highestROCofcelltype[i]]
        )
        ax[value[0], value[1]].plot(
            input.fpr[highestROCofcelltype[i]],
            input.tpr[highestROCofcelltype[i]],
            label="ROC(area = %0.2f)" % (input.roc_auc[highestROCofcelltype[i]]),
        )
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir + "ROC_" + filename + "." + saveas,
    )
    plt.savefig(
        input.niche_pred_outdir + "ROC_" + filename + "." + saveas,
        bbox_inches="tight",
...
def model_log_regression(
    ...
    polynomial = PolynomialFeatures(
        degree=BothLinearAndCrossTerms, interaction_only=True, include_bias=False
    )
    ...
    while flag:
        ...
        pipe_grid = Pipeline(
            [
                ...
                ("logistic_regression_grid", gs_grid),
            ]
        )
    ...
    coef = []
    seed = seed + 1
    sss = RepeatedStratifiedKFold(
        n_splits=K_fold, n_repeats=n_repeats, random_state=seed
    )
    ...
    fpr, tpr, roc_auc = plot_multiclass_roc(
        pipe, x_test, y_test, n_classes=len(classes)
    )
...
def find_interacting_cell_types(
    input,
    choose_celltypes=[],
    ...
    for k in range(len(a)):
        if a[goodPredictedCellType[k]] >= confusion_cutoff:
            ...
            if flag == 1:
                ...
                # fig.tight_layout()
                savefname = remove_extra_character_from_name(
                    nameOfCellType[goodPredictedCellType[k]]
                )
                print(
                    "The figures are saved: ",
                    filename + "/Rank" + str(k + 1) + "_" + savefname + "." + saveas,
                )
                fig.savefig(
                    filename + "/Rank" + str(k + 1) + "_" + savefname + "." + saveas,
                    bbox_inches="tight",
...
def spatial_neighborhood_analysis(
    ...
    **Notes:**
    ...
    BothLinearAndCrossTerms = (
        1  # If only linear terms then put 1; For both linear and crossterms use 2
    )
    ...
    PP, cluster, noct, fraction_CT, clusterWithBarcodeId = reading_data(
        posdata,
        degbased_cluster,
        degbased_ctname,
        output_nico_dir,
    ...
    M, neighbors, distance = create_spatial_CT_feature_matrix(
        Radius, PP, cluster, noct, fraction_CT, niche_pred_outdir, epsilonThreshold
    )
    pickle.dump(
        neighbors, open(output_nico_dir + "neighbors_" + str(Radius) + ".p", "wb")
    )
    pickle.dump(
        distance, open(output_nico_dir + "distances_" + str(Radius) + ".p", "wb")
    )
    ...
    neighborhoodClass, target, inputFeatures = read_processed_data(
        Radius, niche_pred_outdir
    )
    (
        cmn,
        coef,
...
def plot_evaluation_scores(
    input, saveas="pdf", transparent_mode=False, showit=True, dpi=300, figsize=(4, 3)
):
    """
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir + "scores_" + str(input.Radius) + "." + saveas,
    )
    fig.savefig(
        input.niche_pred_outdir + "scores_" + str(input.Radius) + "." + saveas,
        bbox_inches="tight",
...
def plot_niche_interactions_without_edge_weight(
    input,
    niche_cutoff=0.1,
    saveas="pdf",
    transparent_mode=False,
    ...
    edge_labels = dict(
        [((n1, n2), "%0.2f" % G[n1][n2]["weight"]) for n1, n2 in G.edges]
    )
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir
        + "Niche_interactions_without_edge_weights_R"
        + str(input.Radius)
        + "."
        + saveas,
    )
...
def plot_niche_interactions_with_edge_weight(
    input,
    niche_cutoff=0.1,
    saveas="pdf",
    transparent_mode=False,
    ...
    print(
        "The figures are saved: ",
        input.niche_pred_outdir
        + "Niche_interactions_with_edge_weights_R"
        + str(input.Radius)
        + "."
        + saveas,
    )
